...
import type { Dir, FixtureTypeId, Rot } from '../game/types';
import { FRESH, GOODS, PAL, shade } from './palette';
...
function spec(typeId: FixtureTypeId, rot: Rot): SpriteSpec {
  ...
  return { W: fp.w * TILE, H: fp.h * TILE + elev, topH: fp.h * TILE, elev, faces: FIXTURES[typeId].faces.map((d) => rotDir(d, rot)) };
}

function drawShelfLike(ctx: Ctx, typeId: FixtureTypeId, rot: Rot, variant: number) {
  ...
  if (frontIsFace) {
    ...
    for (let r = 0; r < rows; r++) {
      ...
      goodsRow(ctx, 3, y + 1, W - 6, rowH - 3, goods, r + variant);
    }
  } else {
    // 端板：一块价签牌
...
function drawChiller(ctx: Ctx, rot: Rot, variant: number) {
  const def = FIXTURES['upright-chiller'];
  const { W, H, topH, elev, faces } = spec('upright-chiller', rot);
  px(ctx, 0, 0, W, H, PAL.outline);
  px(ctx, 1, 1, W - 2, topH - 2, shade(def.color, 40));
  hline(ctx, 1, 1, W - 2, PAL.white);
  px(ctx, 1, topH, W - 2, elev - 1, def.dark);
  if (faces.includes(2)) {
    // 玻璃门
    ...
    for (let r = 0; r < 3; r++) {
      ...
      goodsRow(ctx, 4, y + 1, W - 8, 3, goods, r);
      hline(ctx, 2, y + 4, W - 4, '#8fdbe8');
    }
    px(ctx, W / 2 - 1, topH + 4, 1, elev - 9, PAL.light);
  } else {
    px(ctx, 3, topH + 2, W - 6, elev - 6, shade(def.dark, -15));
  }
...
function drawIslandFreezer(ctx: Ctx, rot: Rot, variant: number) {
  const def = FIXTURES['island-freezer'];
  ...
  for (let y = 4; y + 3 <= topH - 3; y += 4) goodsRow(ctx, 4, y, W - 8, 3, goods, y);
  px(ctx, 3, 2, 1, topH - 5, PAL.white);
  hline(ctx, 1, topH / 2, W - 2, 'rgba(255,255,255,0.6)');
  // 白色箱体
  px(ctx, 1, topH, W - 2, elev - 1, PAL.white);
  hline(ctx, 1, topH, W - 2, def.color);
  hline(ctx, 1, H - 2, W - 2, PAL.light);
  px(ctx, W - 4, topH + 2, 2, 2, def.dark);
}

function drawWood(ctx: Ctx, rot: Rot, variant: number) {
  const def = FIXTURES['wood-display'];
  const { W, H, topH, elev } = spec('wood-display', rot);
  const fresh = FRESH[variant % FRESH.length];
  px(ctx, 0, 0, W, H, PAL.outline);
  px(ctx, 1, 1, W - 2, topH - 2, shade(def.color, 35));
  // 木板纹
  for (let y = 3; y < topH - 1; y += 4) hline(ctx, 1, y, W - 2, shade(def.color, 10));
  // 四个木箱
  ...
  for (let x = 1; x < W - 1; x += 6) px(ctx, x, topH, 1, elev - 1, def.dark);
  hline(ctx, 1, H - 2, W - 2, def.dark);
}

function drawPromo(ctx: Ctx, variant: number) {
  const def = FIXTURES['promo-display'];
  ...
  px(ctx, 5, topH + 3, W - 10, 1, def.dark);
  px(ctx, 5, topH + 5, W - 10, 1, def.dark);
  hline(ctx, 1, H - 2, W - 2, def.dark);
}
...
/** 取货架 sprite（按类型/旋转/变体/缩放/帧缓存） */
export function fixtureSprite(typeId: FixtureTypeId, rot: Rot, variant: number, scale: number, frame = 0): HTMLCanvasElement {
  const f = typeId === 'checkout' || typeId === 'upright-chiller' ? frame % 2 : 0;
  const key = `${typeId}|${rot}|${variant}|${scale}|${f}`;
  ...
  switch (typeId) {
    case 'upright-chiller':
      drawChiller(ctx, rot, variant);
      if (f === 1) px(ctx, 5, 2, 2, 1, PAL.white);
      break;
    case 'island-freezer':
      drawIslandFreezer(ctx, rot, variant);
      break;
    case 'wood-display':
      drawWood(ctx, rot, variant);
      break;
    case 'promo-display':
      drawPromo(ctx, variant);
      break;
    case 'checkout':
      drawCheckout(ctx, rot, f === 1);
      break;
    default:
      drawShelfLike(ctx, typeId, rot, variant);
  }
  cache.set(key, c);
  return c;
}
...
/** 小人 sprite：6×10，2 帧走路 */
export function personSprite(look: { skin: number; shirt: number; pants: number; hair: number }, frame: number, scale: number): HTMLCanvasElement {
